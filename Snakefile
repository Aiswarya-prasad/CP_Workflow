...
rule all:
    input:
        ...
        directory(expand(os.path.join("QC", "runs", "MinionQC", "{runnames}"), runnames=config['runnames']))
        directory(expand(os.path.join("QC", "runs", "Nanocomp", "{runnames}"), runnames=config['runnames']))
